            ...
            return (
              ...
              >
                <div
                  ...
                  } xxl:gap-20 xl:gap-16 lg:gap-12 gap-10 relative overflow-hidden ${frameworkGridSettings[index].cardGradientBefore} ${frameworkGridSettings[index].cardGradientAfter}`}
                >
                  <div className={`relative z-10 ${FRAMEWORK_DATA.length - 1 === index ? "lg:py-10 pt-6 pe-6" : ""}`}>
                    ...
                    <p className="lg:text-xl md:text-lg text-base font-source-sans font-normal">{description}</p>
                  </div>
                  <img className="relative z-10" src={image} alt={title} /> {/* Updated alt text */}
                </div>
              </motion.div>
            );
          })}
